...
import { groupSystemSync, fetchSupplyPlan, fetchWorkingOrders, clearLocalWorkflowState } from '../utils/api';
...
const SyncBridge = ({ onSyncComplete, showPush = true, showPull = true }) => {
    ...
    const handlePushParameters = async () => {
        try {
            ...
            // Reset step 1.1 if we are re-syncing master data
    ...
    const handlePullPlan = async () => {
        ...
        try {
            ...
            // Note: Keep step 1.1 state here? Usually supply plan follows assortment.
            // But if it's a fresh cycle, we might want to clear it too.
                ...
                {showPull && (
                    <div className={clsx(
                        ...
                        <div className="space-y-4">
                            ...
                            {(step2Status === 'SUCCESS' || (!step2Status.includes('LOCKED') && step2Status !== 'SUCCESS')) && (
                                <div className="flex flex-col items-center">
                                    ...
                                    <button
                                        ...
                                        className={clsx(
                                            "text-[9px] font-black uppercase tracking-widest hover:underline flex items-center gap-1.5 transition-all outline-none",
                                            step2Status === 'SUCCESS' ? "text-blue-200 hover:text-white" : "text-[#003E7E] opacity-60 hover:opacity-100"
                                        )}
                                    >
                        ...
                        {step2Status === 'LOCKED' && !showPush && (
                            <div className="absolute inset-0 bg-white/10 backdrop-blur-[1px] flex items-center justify-center">
                                <div className="flex items-center gap-2 bg-slate-800 text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest">
                                    <Info size={12} className="text-amber-400" /> Complete 1.2.2 in Previous Step First
                                </div>
                            </div>
                        )}
                        {step2Status === 'LOCKED' && showPush && (
